...
diagram]. Nothing in this repo references dengjen-nvda, dengjen-tts, or
dengjen-piper-rs -- as far as this codebase shows, it's a standalone tool.
(dengjen-tts optionally depends on it, feature-gated, from its side.)

== Container

The same core inference logic is exposed through five different frontends:
a CLI, a C ABI, a Python package, a Java library that loads that same C ABI
directly, and (indirectly) dengjen-tts's optional `tashkeel` feature.
